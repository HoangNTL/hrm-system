...
export function useStaffDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      try {
        const attendanceRes = await dashboardAPI.getAttendanceHistory({
      ...
      try {
        const payrollRes = await dashboardAPI.getPayslip({
      ...
      try {
        const reqRes = await dashboardAPI.getMyAttendanceRequests({
          page: 1,
          limit: 100,
        });
