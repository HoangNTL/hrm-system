...
import { Icon, Pagination, Table } from '@/shared/components';
...
export default function UserTable({
  ...
  const formatDate = (date) => {
    if (!date) {
      return <span className="text-secondary-500 dark:text-secondary-400 italic">Never</span>;
    }
  ...
  const handleCheckboxChange = useCallback((user, event) => {
  ...
  const columns = [
    {
      ...
      render: (_cell, user) => (
        <input
          ...
          onChange={(event) => handleCheckboxChange(user, event)}
          className="w-4 h-4 rounded border-secondary-300 dark:border-secondary-600 text-primary-600 focus:ring-primary-500 dark:bg-secondary-800"
        />
      ),
      width: '50px',
    ...
    },
    {
      ...
      render: (cell) => (cell ? (
        ...
        <span className="text-secondary-500 dark:text-secondary-400 italic text-sm">
          Not linked
        </span>
      )),
    },
