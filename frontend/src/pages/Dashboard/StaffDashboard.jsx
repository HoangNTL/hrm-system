...
export default function StaffDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setStats({ workDays, lateCount, onTimeRate, upcomingPayroll });
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-4xl font-bold text-purple-900 dark:text-purple-100 mt-2">
                {stats.upcomingPayroll > 0 ? `${(stats.upcomingPayroll / 1000000).toFixed(1)}M` : '0'}
              </p>
            </div>
        ...
        {attendanceTrends.length === 0 ? (
          ...
          <div className="space-y-4">
            {attendanceTrends.map((trend, index) => {
              ...
              return (
                <div key={index} className="flex items-center gap-4 p-4 rounded-lg bg-secondary-50 dark:bg-secondary-900/50 border border-secondary-100 dark:border-secondary-800">
                  <div className={`w-12 h-12 rounded-full ${config.color} flex items-center justify-center flex-shrink-0`}>
                    <Icon name={config.icon} className="w-6 h-6 text-white" />
                  </div>
                  <div className="flex-1">
                    <p className="font-semibold text-secondary-900 dark:text-secondary-50">
                      {trend.day} - {formatDate(trend.date)}
                    </p>
                    <p className="text-sm text-secondary-600 dark:text-secondary-400 mt-1">
                      {trend.checkIn ? `Check-in: ${trend.checkIn}` : 'No check-in'}
                      {trend.checkOut && ` • Check-out: ${trend.checkOut}`}
                    </p>
                  </div>
                  <span className={`px-4 py-2 rounded-lg text-sm font-semibold ${config.textColor} ${config.color}/20`}>
                    {config.label}
                  </span>
                </div>
              );
